...
const ACTION_FR: Record<string, string> = {
  ...
  'Désactivation du mode simulation': 'Désactivation du mode simulation',
};
...
const SimulationLogsPage = () => {
  ...
  const handleExportCSV = () => {
    const headers = ['Utilisateur', 'Action', 'Catégorie', 'Date/Heure', 'Détails'];
      ...
      <div className="flex flex-wrap gap-4 mb-4 items-end">
        <div>
          ...
          <select className="border rounded px-2 py-1" value={userFilter} onChange={e => setUserFilter(e.target.value)}>
            ...
            {unique(logs.map(l => l.user_name)).map(u => <option key={u} value={u}>{u}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-sm font-medium mb-1">Action</label>
          <select className="border rounded px-2 py-1" value={actionFilter} onChange={e => setActionFilter(e.target.value)}>
            <option value="">Toutes</option>
            {unique(logs.map(l => l.action)).map(a => <option key={a} value={a}>{ACTION_FR[a] || a}</option>)}
          </select>
        </div>
        <div>
          ...
          <select className="border rounded px-2 py-1" value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)}>
            <option value="">Toutes</option>
            {unique(logs.map(l => l.category)).map(c => <option key={c} value={c}>{CATEGORY_FR[c] || c}</option>)}
          </select>
        </div>
          ...
          <tbody>
            {loading ? (
              ...
              filteredLogs.map((log, idx) => (
                <tr key={log.id || idx} className="border-t last:border-b">
                  <td className="py-2 px-4 font-mono text-xs">{log.user_id || '-'}</td>
